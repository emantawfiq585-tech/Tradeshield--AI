...
class Database:
    def __init__(self):
        self.conn = sqlite3.connect('tradeshield.db', check_same_thread=False)
    ...
    def is_vip(self, user_id):
        ...
        if not user[4]:
            return False
        expiry = datetime.strptime(user[4], '%Y-%m-%d').date()
        return expiry >= datetime.now().date()
    
    def activate_vip(self, user_id, days=30):
        expiry = (datetime.now() + timedelta(days=days)).strftime('%Y-%m-%d')
        self.cursor.execute('''
            UPDATE users SET is_vip = TRUE, vip_expiry = ?
            WHERE user_id = ?
        ''', (expiry, user_id))
        self.conn.commit()
    ...
    def can_check(self, user_id):
        ...
        if last_check != str(today):
            self.cursor.execute('''
                UPDATE users SET daily_checks = 1, last_check = ?
                WHERE user_id = ?
            ''', (str(today), user_id))
            self.conn.commit()
            return True
